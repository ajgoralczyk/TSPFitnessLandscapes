...
class LK:
    ...
    def closest_tuples(self):
        def closest_tuple(node1, node2, indexes):
        ...
        for i in range(self.n):
            closest[i] = list(starmap(closest_tuple, [(i, node, indexes) for node in self.closest_nodes[i]]))
        return closest
        ...
        for index_t1, t1 in enumerate(self.tour):  # Step 2, 12
            ...
            for t2, index_t2 in around_t1:  # Step 3, 11
                ...
                for t3, (_, Gi, index_t3) in closest_neighbours:
                    ...
                    is_better, better_path, better_path_cost = self.chooseX(solutions, intersection, closest_tuples, t1, index_t1, t3, index_t3, Gi, broken, joined, 2)
                    if is_better:
    ...
    def chooseX(self, solutions, intersection, closest_tuples, t1, index_t1, last, index_last, gain, broken, joined, depth):
        # if len(broken) >= 9: ############################################# 1
        ...
        for t2i, index_t2i in around:
            ...
            if xi not in joined and xi not in broken and t2i != t1:
                ...
                if is_tour:
                    ...
                    choice, new_tour, new_tour_cost = self.chooseY(solutions, intersection, closest_tuples, t1, index_t1, t2i, new_gain, broken | {(last, t2i),(t2i, last)}, joined, depth)
    ...
    def chooseY(self, solutions, intersection, closest_tuples, t1, index_t1, t2i, gain, broken, joined, depth):
        closest_neighbours = self.closest(t2i, gain, broken, closest_tuples[t2i])
        for node, (_, Gi, index_node) in closest_neighbours:
            is_better, new_tour, new_tour_cost = self.chooseX(solutions, intersection, closest_tuples, t1, index_t1, node, index_node, Gi, broken, joined | {(t2i, node), (node, t2i)}, depth+1)
