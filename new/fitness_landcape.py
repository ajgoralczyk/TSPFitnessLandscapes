...
def generate_LON(A, runs_amount, termination_criterion, kick_strength):  # LON sampling (Algorithm 1 from paper [1])
    ...
    begin = time.time()
    for s_res, s, iters_, L_, E_ in starmap(CLK, [(I, A, kick_strength) for t in range(T)]):  # starmap - map with multiple args
        L = L | L_  # set union
        for k, v in E_.items():
            if k in E:
                E[k] += v
...
def CLK(I, A, K):  # Iterated local search Chained-LK
    ...
    s = np.array(optimise_path(np.random.permutation(n).tolist(), A, n))    # random path turned into local optimum
    ...
    while i < I:
        ...
        p = np.array(optimise_path(p.tolist(), A, n))
        p_res = path_length(p, A, n)
        i += 1
        if p_res < s_res:  # TODO difference from article
