...
def closest_nodes(A, n):
    ...
    for i in range(n):
        nodes = list(range(n))
        closest[i] = sorted(nodes, key=lambda n: A[i,n])[1:6]
    return closest
...
def generate_LON(A, runs_amount, termination_criterion, kick_strength):  # LON sampling (Algorithm 1 from paper [1])
    ...
    E = {}  # escape edges
    L_in_degree = {}
    closest = closest_nodes(A, A.shape[0])
    ...
    begin = time.time()
    for s_res, s, iters_, L_, E_ in starmap(CLK, [(I, A, kick_strength, closest) for t in range(T)]):
        L = L | L_
        for k, v in E_.items():
            if k in E:
                E[k] += v
...
def CLK(I, A, K, closest_nodes):  # Iterated local search Chained-LK
    ...
    lk = LK(np.random.permutation(n).tolist(), A, n, closest_nodes)
    s = np.array(lk.optimize_path())    # random path turned into local optimum
    ...
    while i < I:
        ...
        lk = LK(p.tolist(), A, n, closest_nodes)
        p = np.array(lk.optimize_path())
