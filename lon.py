...
def generate_LON(A, n, runs_amount, termination_criterion, kick_strength):  # LON sampling (Algorithm 1 from paper [1])
    ...
    begin = time.time()
    for s_res, s, iters_, L_, E_, in_degree in pool.starmap(CLK, [(I, A, n, kick_strength, closest) for t in range(T)]):
        L = L | L_
        for k, v in E_.items():
            if k in E:
                E[k] += v
        ...
        for optimum in in_degree.keys():
            if optimum in L_in_degree.keys():
                L_in_degree[optimum] += in_degree[optimum]
            else:
                L_in_degree[optimum] = in_degree[optimum]

        if s_res in run_result:
            run_result[s_res] += 1 ############## TODO something wrong with success rate
            iters[s_res] += iters_ ############## TODO iters?
        else:
            run_result[s_res] = 1
            iters[s_res] = iters_
...
def CLK(I, A, n, K, closest_nodes):  # Iterated local search Chained-LK
    ...
    random_path = random.permutation(n).tolist()
    s = np.array(lk.optimize_path(random_path))    # random path turned into local optimum
    ...
    L_in_degree = {local_optimum: 1}
    i = 0
    total_iters = 0
    while i < I:
        p = perturbate(s, K)  # K double-bridge operations
        p = np.array(lk.optimize_path(p.tolist())) # lin-kernighan
        ...
        if p_res < s_res:  # difference from article [1]
            ...
            if new_local_optimum in L_in_degree.keys():
                L_in_degree[new_local_optimum] += 1
            else:
                L_in_degree[new_local_optimum] = 1
            if e in E:
                E[e] += 1
            ...
            total_iters += i
            i = 0
    return s_res, s, total_iters, L, E, L_in_degree


def perturbate(nodes_order, K):  # K - kick strength (how many double-bridge operations are applied)
    def double_bridge(path):
        random = np.random.RandomState()
        s = np.sort(random.choice(path.size, 3, replace=False))  # numpy.random.choice(int, sampleSize, copies?)
...
def LONs_generation(instances_foldername, lons_foldername, metrics_foldername, K, termination_criterion):
    for filename in os.listdir(instances_foldername):
        if filename.endswith('.matrix'):
            ...
            L, E, best_path_length, best_path, successes, mean_iters, L_in_degree = generate_LON(A, n, 1000, termination_criterion, K)
...
if __name__ == '__main__':
    ...
    LONs_generation("checking_data", "checking/1_lons", "checking/3_performance_metrics", 1, 100) # FOR LOCAL TESTS
